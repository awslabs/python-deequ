...
class TestAnalyzers:
    ...
    def ApproxQuantiles(self, column, quantiles):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(ApproxQuantiles(column, quantiles)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Completeness(self, column, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Completeness(column, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Compliance(self, instance, predicate, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Compliance(instance, predicate, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Correlation(self, column1, column2, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Correlation(column1, column2, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        return result_df.select('value').collect()
    ...
    def CountDistinct(self, columns):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(CountDistinct(columns)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Datatype(self, column, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(DataType(column, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Distinctness(self, columns, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Distinctness(columns, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def Entropy(self, column, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Entropy(column, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def test_KLLSketch(self):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(KLLSketch('b', KLLParameters(self.spark, 2, 0.64, 2))) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_df.show()
        return result_df.select('value').collect()
    ...
    def test_ApproxQuantile(self):
        self.assertEqual(self.ApproxQuantile('b', .5), [Row(value=2.0)])
        self.assertEqual(self.ApproxQuantile('c', .5), [Row(value=5.0)])
        self.assertEqual(self.ApproxQuantile('b', .25), [Row(value=1.0)])

    def Uniqueness(self, columns, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(Uniqueness(columns, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def UniqueValueRatio(self, columns, where=None):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(UniqueValueRatio(columns, where)) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
    ...
    def test_PatternMatch(self):
        result = self.AnalysisRunner.onData(self.df) \
            .addAnalyzer(PatternMatch(column='a', pattern_regex='ba(r|z)')) \
            .run()
        result_df = AnalyzerContext.successMetricsAsDataFrame(self.spark, result)
        result_json = AnalyzerContext.successMetricsAsJson(self.spark, result)
        df_from_json = self.spark.read.json(self.sc.parallelize([result_json]))
